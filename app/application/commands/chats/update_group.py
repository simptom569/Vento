...
class UpdateGroupHandler:
    ...
    async def handle(self, cmd: UpdateGroupCommand) -> None:
        ...
        if not user:
            raise ForbiddenError("Вы не состоите в данной Группе")
        
        if user.role not in (ChatRole.OWNER,):
            raise ForbiddenError("Недостаточно прав на изменение Группы")
